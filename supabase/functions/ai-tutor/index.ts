...
// ── Критерии оценивания сочинения (задание 27, ЕГЭ 2027) ──
const ESSAY_RUBRIC = `
КРИТЕРИИ ОЦЕНИВАНИЯ СОЧИНЕНИЯ (задание 27, ЕГЭ 2027, максимум 22 балла):
...
async function gradeEssay(essay: string, source: string) {
  ...
  const system =
    ...
    `Оцениваешь сочинение (задание 27) ТОЧНО по официальным критериям ФИПИ 2027. ` +
  ...
  try {
    ...
    if (mode === "answer") {
      ...
      const system =
        ...
        `опираясь на материал урока и нормы ФИПИ 2027. Скажи: что верно, где ошибки (конкретно), ` +
    ...
    const courseTitle = lesson.course_slug === "russian"
      ? "Русский язык ЕГЭ 2027" : "Информатика ЕГЭ 2027";
